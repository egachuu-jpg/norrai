...
- NEVER put `continueOnFail: true` (or `onError: "continueRegularOutput"`) on a send/action node (SendGrid/HTTP email, Twilio SMS, etc.) — it converts a hard failure into a silent no-op: the node reports success, the execution finishes green, the Error Workflow (Error Logger) never fires, and the sequence marches forward sending nothing. `continueOnFail` belongs on *logging/lookup* nodes only. Send nodes should use `onError: "stopWorkflow"` (the default) so failures throw → Error Logger writes a `failed` event + Slack alert with `last_node` naming the step. Root-caused 2026-06-25: 9 cold-nurture sequences silently swallowed SendGrid 401 "Maximum credits exceeded" (free trial lapsed) because `Email T1–T6` had `continueOnFail: true`
- `'{{ $('NodeName').first().json.field }}'` inside a SQL string causes "invalid sequence" — the `'` inside `$('NodeName')` conflicts with the surrounding SQL single-quote delimiters; extract values into `$json` via an upstream Code node and use `{{ $json.field }}` in the query instead
- n8n API (`n8n_update_full_workflow`) silently drops the `queryParams` option from Postgres nodes — parameterized queries (`$1`, `$2`) deploy without bound values and fail at runtime; use a Code node to extract values into `$json`, then reference with `{{ $json.field }}` in plain SQL
- n8n Cloud project credential scoping: a credential visible in the node UI editor is NOT usable at runtime until explicitly shared with the project via Credentials → (credential) → Sharing → add project; the error message is "Node X does not have access to the credential"
- `respondToWebhook` continues executing downstream nodes after the HTTP response is sent — it's safe to put logging nodes after Respond: Success in the chain; the response fires first and execution continues in the background
...
- Claude model IDs are hardcoded inline in every HTTP Request node's `jsonBody` (`"model": "..."`) and duplicated across nodes (6 per nurture workflow, 13 workflows total) — when a model retires (e.g. `claude-sonnet-4-20250514` retired 2026-06-15) every node 404s with "The resource you are requesting could not be found"; the swap to `claude-sonnet-4-6` is a multi-file find/replace, so a central model Set node would avoid the scramble next time
- The Workflow Error Logger payload captures execution metadata only (`execution_id`, `last_node`, `error_message`) — NOT lead identity; to recover which leads a failed batch hit, match the failure timestamp + count against `leads.nurture_enrolled_at`, not the `workflow_events` payload
...
- This n8n Cloud instance (draft/versioning model) **silently drops node `parameters` edits made via the public API** — MCP `updateNode`/`patchNodeField` return `saved:true` and a direct REST `PUT /workflows/{id}` returns 200, but node params revert on read (proven with a control edit to `options.timeout`). Only node `credentials` and workflow `settings` persist via API. Node parameter changes must be made in the n8n UI (this qualifies the earlier dot-path `updateNode` note — the syntax is right but the write still won't stick for params)
- REST `POST /workflows` (create) DOES persist node params even though `PUT` (update) doesn't — to script a modified copy of a workflow, create a fresh one from the edited JSON rather than updating in place; move it to the right project with the `transferWorkflow` op (`destinationProjectId`, e.g. Norr AI `dHMe2aoOwTztDaWE`)
- n8n public API `PUT`/`POST` rejects `settings` containing keys outside the allowed set (e.g. `binaryMode`) → 400 "settings must NOT have additional properties"; filter to `executionOrder,timezone,errorWorkflow,saveDataErrorExecution,saveDataSuccessExecution,saveExecutionProgress,saveManualExecutions,executionTimeout` before sending
- `n8n_manage_credentials` create has no `projectId` param → credentials land in the API user's **Personal** project, invisible to team-project (Norr AI) workflows; create project-scoped credentials in the UI, or move/copy them after
- HTTP Request node with `contentType: "raw"` blanks the outgoing `Content-Type` header unless `rawContentType` is set — a manually-added `content-type` entry in headerParameters does NOT survive, so the receiving API sees an empty Content-Type (Apify returned 400 "contentType is required Received \"\""); use `contentType: "json"` + `jsonBody` for JSON bodies
...
- Weichert listing pages (`northstar.weichert.com`) are a client-side SPA — the photo is absent from static HTML (no `og:image`, no image URLs, no JSON data islands), so static scrapers (native fetch AND Apify cheerio-scraper) return `null`; cheerio-scraper also requires full-account-permission approval and still couldn't render it. Listing photos need a JS-rendering scraper or the undocumented `weichertapi.azurewebsites.net` API; the Weekly Marketing Drip Send already degrades gracefully to address-only cards via `onError: continueRegularOutput`, so address-only is the zero-dependency default
...
- Gmail auto-forwarding rules preserve the original `From:` header — a forwarded email at hello@norrai.co still shows `from:no-reply@boldtrail.com`, not the agent who forwarded it; `from:agent@email.com` filters won't match; use subaddresses (`hello+evan@norrai.co`) and filter by `to:` instead for reliable per-agent routing
...
- Central inbox forwarding (agents forward to hello@norrai.co) requires only one Gmail OAuth credential vs. one per-agent; preferred over per-agent OAuth when agents are on a brokerage-managed Google Workspace where credential sharing is impractical; routing by from: address requires testing actual forwarding behavior first (see auto-forward gotcha above)
- Unsubscribe routing split: `nurture-deenroll-confirm` removes a lead from the nurture sequence (`status = 'unenrolled'`, uses `lead_id` param); `Email Unsubscribe Handler` sets `email_opt_out = TRUE` on the lead for all one-off emails (ILR, Review Request, Open House Follow-Up) — they serve different purposes and must not be swapped
- Email unsubscribe links in outbound emails must point to `tools.norrai.co/unsubscribe?email=...` (the webpage), not directly to the n8n handler — direct browser→n8n cross-origin POST triggers CORS preflight failure; the webpage's JS fetch handles it correctly
